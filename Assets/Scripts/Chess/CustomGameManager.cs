...
using Core.UnityChess;
using Core.UnityChess.Game;
	...
	public async void StartNewGame(bool isWhiteAI = true, bool isBlackAI = true) {
...
#endif
			game = new global::UnityChess.Core.UnityChessLib.src.Base.Game();
		...
		private bool TryExecuteMove(Movement move) {
			...
			if (latestHalfMove.CausedCheckmate || latestHalfMove.CausedStalemate) {
				CustomBoardManager.Instance.SetActiveAllPieces(false);
				GameEndedEvent?.Invoke();
			} else {
				CustomBoardManager.Instance.EnsureOnlyPiecesOfSideAreEnabled(SideToMove);
			}
		...
		private async Task<bool> TryHandleSpecialMoveBehaviourAsync(SpecialMove specialMove) {
			switch (specialMove) {
				case CastlingMove castlingMove:
					CustomBoardManager.Instance.CastleRook(castlingMove.RookSquare, castlingMove.GetRookEndSquare());
					return true;
				case EnPassantMove enPassantMove:
					CustomBoardManager.Instance.TryDestroyVisualPiece(enPassantMove.CapturedPawnSquare);
					return true;
				case PromotionMove { PromotionPiece: null } promotionMove:
					...
					UIManager.Instance.SetActivePromotionUI(false);
					CustomBoardManager.Instance.SetActiveAllPieces(true);
					...
					CustomBoardManager.Instance.TryDestroyVisualPiece(promotionMove.Start);
					CustomBoardManager.Instance.TryDestroyVisualPiece(promotionMove.End);
					CustomBoardManager.Instance.CreateAndPlacePieceGO(promotionMove.PromotionPiece, promotionMove.End);
		...
		private async void OnPieceMoved(Square movedPieceInitialSquare, Transform movedPieceTransform, Transform closestBoardSquareTransform, Piece promotionPiece = null) {
			...
			if ((move is not SpecialMove specialMove || await TryHandleSpecialMoveBehaviourAsync(specialMove))
				...
				if (move is not SpecialMove) { CustomBoardManager.Instance.TryDestroyVisualPiece(move.End); }

				if (move is PromotionMove) {
					movedPieceTransform = CustomBoardManager.Instance.GetPieceGOAtPosition(move.End).transform;
				}
		...
		private void DoAIMove(Movement move) {
			GameObject movedPiece = CustomBoardManager.Instance.GetPieceGOAtPosition(move.Start);
			GameObject endSquareGO = CustomBoardManager.Instance.GetSquareGOByPosition(move.End);
			OnPieceMoved(
				move.Start,
