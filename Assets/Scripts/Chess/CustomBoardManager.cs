...
using UnityEngine;
using static UnityChess.Core.UnityChessLib.src.Base.SquareUtil;

namespace Core.Chess
{
	public class CustomBoardManager : MonoBehaviourSingleton<CustomBoardManager> {
		...
		private void Awake() {
			UnityChess.Game.GameManager.NewGameStartedEvent += OnNewGameStarted;
			UnityChess.Game.GameManager.GameResetToHalfMoveEvent += OnGameResetToHalfMove;
		...
		private void OnNewGameStarted() {
			...
			foreach ((Square square, Piece piece) in UnityChess.Game.GameManager.Instance.CurrentPieces) {
				CreateAndPlacePieceGO(piece, square);
			}

			EnsureOnlyPiecesOfSideAreEnabled(UnityChess.Game.GameManager.Instance.SideToMove);
		}
		...
		private void OnGameResetToHalfMove() {
			...
			UnityChess.Game.GameManager.Instance.HalfMoveTimeline.TryGetCurrent(out HalfMove latestHalfMove);
			if (latestHalfMove.CausedCheckmate || latestHalfMove.CausedStalemate) SetActiveAllPieces(false);
			else EnsureOnlyPiecesOfSideAreEnabled(UnityChess.Game.GameManager.Instance.SideToMove);
		}
		...
		public void EnsureOnlyPiecesOfSideAreEnabled(Side side) {
			...
			foreach (VisualPiece pieceBehaviour in visualPiece) {
				Piece piece = UnityChess.Game.GameManager.Instance.CurrentBoard[pieceBehaviour.CurrentSquare];
			
				pieceBehaviour.enabled = pieceBehaviour.PieceColor == side
				                         && UnityChess.Game.GameManager.Instance.HasLegalMoves(piece);
			}
		}
